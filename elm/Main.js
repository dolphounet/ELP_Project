...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aM, request.aw, true);
		} catch (e) {
...
var $author$project$Main$NewNumber = function (a) {
	return {$: 1, a: a};
};
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{aC: false, aE: r.aE, X: r.X, Z: r.Z, aM: r.aM, aX: r.aX, av: r.av, aw: r.aw}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{aE: $elm$http$Http$emptyBody, X: r.X, Z: _List_Nil, aM: 'GET', aX: $elm$core$Maybe$Nothing, av: $elm$core$Maybe$Nothing, aw: r.aw});
};
...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{x: $author$project$Main$Loading, C: $author$project$Main$Loading, E: 1, F: false, H: _List_Nil, u: '', w: ''},
...
var $author$project$Main$SuccessDef = function (a) {
	return {$: 3, a: a};
};
var $author$project$Main$extract_from_list = F2(
	function (lst, indice) {
		extract_from_list:
		while (true) {
			if (!indice) {
...
var $author$project$Main$wordDecoder = $elm$json$Json$Decode$list($author$project$Main$defDecoder);
var $author$project$Main$getRandomDef = function (word) {
	return $elm$http$Http$get(
		{
			X: A2($elm$http$Http$expectJson, $author$project$Main$GotDef, $author$project$Main$wordDecoder),
			aw: 'https://api.dictionaryapi.dev/api/v2/entries/en/' + word
		});
};
var $author$project$Main$len = function (lst) {
	if (!lst.b) {
		return 0;
	} else {
		var x = lst.a;
		var xs = lst.b;
		return 1 + $author$project$Main$len(xs);
	}
};
		...
		switch (msg.$) {
			...
			case 0:
				...
				if (!result.$) {
					...
					var text_list = A2($elm$core$String$split, ' ', fullText);
					var newWord = A2($author$project$Main$extract_from_list, text_list, model.E);
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								C: $author$project$Main$Success(fullText),
								H: text_list,
								w: newWord
							}),
						$elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									...
									A2(
										...
										1,
										$author$project$Main$len(text_list))),
									$author$project$Main$getRandomDef(newWord)
								])));
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{C: $author$project$Main$Failure}),
						$elm$core$Platform$Cmd$none);
				}
			case 2:
				var word = msg.a;
				...
				if (!result.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								x: $author$project$Main$SuccessDef(def)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{x: $author$project$Main$Failure}),
						$elm$core$Platform$Cmd$none);
				}
			case 4:
				var newWord = A2($author$project$Main$extract_from_list, model.H, model.E);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{x: $author$project$Main$Loading, u: '', w: newWord}),
					$elm$core$Platform$Cmd$batch(
						_List_fromArray(
							[
								A2(
								...
								A2(
									$elm$random$Random$int,
									1,
									$author$project$Main$len(model.H))),
								$author$project$Main$getRandomDef(newWord)
							])));
			default:
				return _Utils_Tuple2(
...
var $author$project$Main$NewWord = {$: 4};
var $author$project$Main$SolChange = {$: 5};
...
var $author$project$Main$definitionParsing = function (definitions) {
	if (definitions.b) {
		var x = definitions.a;
		var xs = definitions.b;
		return A2(
			$elm$core$List$cons,
			A2(
				$elm$html$Html$li,
			...
			$author$project$Main$definitionParsing(xs));
	} else {
		return _List_Nil;
	}
};
var $author$project$Main$meaningParsing = function (meanings) {
	if (meanings.b) {
		var x = meanings.a;
		var xs = meanings.b;
		return A2(
			...
			A2(
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$ol,
						_List_Nil,
						$author$project$Main$definitionParsing(x.U))
					])),
			$author$project$Main$meaningParsing(xs));
	} else {
		return _List_Nil;
	}
};
var $elm$html$Html$ul = _VirtualDom_node('ul');
var $author$project$Main$wordParsing = function (defs) {
	if (defs.b) {
		var x = defs.a;
		var xs = defs.b;
		return A2(
			...
			A2(
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$ul,
						_List_Nil,
						$author$project$Main$meaningParsing(x.ac))
					])),
			$author$project$Main$wordParsing(xs));
	} else {
		return _List_Nil;
	}
};
